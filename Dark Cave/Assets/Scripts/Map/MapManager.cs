...
namespace MapSystem
{
    ...
    public class MapManager
    {
        ...
        void CreateMinirals()
        {
            ...
            List<List<Coord>> chunksSizes = regions.GetRegions(tileMap, width, height, TileType.Ground);

            int mineralAmount = 0;

            //Loop throught the tile lists and check for size and then start populating the tile map with the miniral
            foreach (List<Coord> chunkList in chunksSizes)
            {
                ...
                if (chunkList.Count > 30)
                {
                    ...
                    for (int i = 0; i < mineralAmount; i++)
                    {
                        ...
                        for (int x = chunkList[randomPoint].tileX - (mineralDepositeSize - mineralDepositeSize + mineralSizeIncrease); x < chunkList[randomPoint].tileX + (mineralDepositeSize - mineralDepositeSize + mineralSizeIncrease); x++)
                        {
                            for (int y = chunkList[randomPoint].tileY - (mineralDepositeSize - mineralDepositeSize + mineralSizeIncrease); y < chunkList[randomPoint].tileY + (mineralDepositeSize - mineralDepositeSize + mineralSizeIncrease); y++)
                            {
                                if (IsInMapRange(x, y))
                                {
                                    if (tileMap[x, y].Type == TileType.Ground)
        ...
        {
            //Here we get the list of cordinate lists for the regions that are ground
            List<List<Coord>> chunksSizes = regions.GetRegions(tileMap, width, height, TileType.Floor);
            ...
            foreach (List<Coord> chunkList in chunksSizes)
            {
                ...
                if (chunkList.Count < 40 && chunkList.Count > 10)
                {
                    ...
                    {
                        tileMap[chunkList[i].tileX, chunkList[i].tileY].Room = RoomType.Tresuary;
                        tileMap[chunkList[i].tileX, chunkList[i].tileY].Type = TileType.Room;
                    }
                }
            }
